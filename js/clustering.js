...
function loadFromFile(fileInput) {
  try {
    ...
    snackbar_label.innerHTML =
      "Unable to load file: not a valid CADS data file.";
...
function extractFeatures() {
  ...
  fs.readdir(rootDir, function (err, files) {
    ...
    // makeImages(); // debug to show single orb feature match
...
function makeImages() {
  const matchFeatures = ({ img1, img2, detector, matchFunc }) => {
    ...
    const dist = Math.abs(
      bestMatches.reduceRight((acc, curr) => acc + Math.abs(curr.distance), 0) /
        bestMatches.length
    );
...
function userDefineFeatureRadius() {
  ...
  remote.getGlobal("shared").featureRadiusImage = path.join(
    rootDir,
    fs.readdirSync(rootDir)[0]
  );
...
function userDefineBlurRadius() {
  ...
  // Set image path for modal to read
  console.log(path.join(rootDir, fs.readdirSync(rootDir)[0]));
  remote.getGlobal("shared").blurRadiusImage = path.join(
    rootDir,
    fs.readdirSync(rootDir)[0]
  );
...
function annotate(cluster) {
  ...
  if (cluster.isLeaf) {
    ...
    cluster.members = cluster.children
      .map((x) => annotate(x))
...
function save() {
  ...
  descriptors.forEach((desc) => {
    const descOutBase64 = cv
      .imencode(".jpg", desc.descriptors)
      .toString("base64"); // Perform base64 encoding
...
function load(saveFile) {
  ...
  // Draw the dendrogram
  dendrogram.drawDendrogram(
    "#dendrogram",
    parsedJSON.clustering,
    descr,
    parsedJSON.rootDir
  );
}
